...
def construct_bystep(e_xu, sens, args):
    ...
    for i in tqdm(range(step)):
        ...
        if (sens[start] == 0):
            ...
            sens_step = sens
        real_size=end-start
        sens_same = sens_step.repeat(end - start, 1)
        sens_diff = sens_step.repeat(end - start, 1)
        idx_one = sens_step[start:end].nonzero(as_tuple=False).squeeze()
        idx_zero = np.delete(np.arange(real_size), idx_one)
        sens_diff[idx_one] = (1 - sens_diff[idx_one])
        sens_same[idx_zero] = (1 - sens_same[idx_zero])

        x_norm_step = x_norm[start:end]
        like_dense = x_norm_step @ x_norm.T

        like_dense =  (1* (like_dense - min_arr[start:end,:])/(max_arr[start:end,:]-min_arr[start:end,:]))
        ...
        diff_like=diff_like.numpy()
        same_unlike=same_unlike.numpy()
...
def sample(pos_all_list, neg_all_list, n_users, args):
    ...
    pos_select_list = np.concatenate([np.random.choice(user_pos_list, size=args.k, replace=False) for user_pos_list in pos_all_list])
    neg_select_list = np.concatenate([np.random.choice(user_neg_list, size=args.k, replace=False) for user_neg_list in neg_all_list])

    return torch.from_numpy(pos_select_list).long(), torch.from_numpy(neg_select_list).long()
...
def train_semigcn(gcn, gcn2, sens, n_users,n_items, e_xu, e_xi, args,classes_num, device='cuda:0'):
    ...
    for i in range(n_users):
        ...
        if(len(pos_all_list[i])<args.k):
            pos_all_list[i]=torch.from_numpy(np.random.choice(pos_all_list[i], size=args.k, replace=True))
        if(len(neg_all_list[i])<args.k):
            neg_all_list[i]=torch.from_numpy(np.random.choice(neg_all_list[i], size=args.k, replace=True))
    ...
    for epoch in tqdm(range(args.sim_epochs)):
        ...
        sim=sim_loss.item()
        if(epoch%100==0):
            print('epoch: %d, classify_loss: %.6f,sim: %.6f' % (epoch, final_loss,sim))
    ...
    for epoch in tqdm(range(args.sim_epochs)):
        ...
        classify_loss = compute_cross_entropy(su2, sens_opp)
        ...
        diff_loss=criterion(si2.squeeze(),si_opp.squeeze())
        ...
        sim=sim_loss.item()       
        if(epoch%100==0):
            print('epoch: %d, classify_loss: %.6f,diff: %.6f,sim: %.6f' % (epoch, final_loss,diff,sim))
...
def train_unify_mi(sens_enc, sens_enc2, inter_enc, club, club2, e_xu, e_xi, dataset, u_sens,
    ...
    print(torch.nonzero(1 - p_su1.eq(p_su2).int()).view(-1))
    print(torch.nonzero(1 - p_si1.eq(p_si2).int()).view(-1))
    best_perf = 0.0
    for epoch in range(args.num_epochs):
        ...
        for uij in train_loader:
            ...
            cos_loss3=torch.cosine_similarity(e_zi[torch.unique(i)], e_si2[torch.unique(i)], dim=1)
            cos_loss4=torch.cosine_similarity(e_zi[torch.unique(i)], e_si[torch.unique(i)], dim=1)
            equal_i=torch.mean((cos_loss3-cos_loss4).abs())
            equal_loss=equal_u*args.equ_reg+equal_i*args.eqi_reg
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='lastfm_bpr_fairness',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    current_GMT = time.gmtime()
    torch.set_printoptions( precision=4,edgeitems=10,sci_mode=False)
    ...
    parser.add_argument('--log_path', type=str, default='logs/lastfm/bprmf/bpr_fairness_lastfm_' + str(time_stamp) + '.txt')
    parser.add_argument('--param_path', type=str, default='param/lastfm/bprmf/bpr_fairness_lastfm_' + str(time_stamp) + '.pth')
    parser.add_argument('--pretrain_path', type=str, default='param/bpr_base_lastfm.pth')
    parser.add_argument('--lreg', type=float, default=1.0)
    parser.add_argument('--ureg', type=float, default=1.0)
    ...
    parser.add_argument('--eqi_reg', type=float, default=1.0)
    parser.add_argument('--u_threshold', type=float, default=0.70)
